...
class IngestionValidator:
    ...
    def _validate_header_integrity(self, extension: str, content: bytes) -> None:
        ...
        if extension in {".jpg", ".jpeg"}:
            # Accept both real JPEGs and PNGs saved with a .jpg extension
            if len(content) < 4:
                raise ValueError("Invalid JPEG header")
            is_jpeg = content[:2] == JPEG_MAGIC
    ...
    def _validate_modality_consistency(self, extension: str, content_type: str | None) -> None:
        ...
        if extension in {".jpg", ".jpeg"}:
            # Accept image/jpeg and image/png — evaluation files are PNGs with .jpg extension
            if (
                "jpeg" not in normalized
                and "jpg" not in normalized
                and "png" not in normalized
            ):
